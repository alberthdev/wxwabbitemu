...
static void link_wait(CPU_t *cpu, time_t tstates) {
	...
	while (tc_tstates(cpu->timer_c) < time_end) CPU_step(cpu);
}
...
static void link_send(CPU_t *cpu, u_char byte) {
	...
	for(bit = 0; bit < 8; bit++, byte >>= 1) {
		...
		for(i = 0; i < LINK_TIMEOUT && vlink != 0; i+=LINK_STEP) link_wait(cpu, LINK_STEP);
		if (vlink != 0) longjmp(exc_byte, LERR_TIMEOUT);

		vout = 0;
		for(i = 0; i < LINK_TIMEOUT && vlink != 3; i+=LINK_STEP) link_wait(cpu, LINK_STEP);
...
static u_char link_target_ID(CPU_t *cpu) {
	switch (cpu->pio.model) {
		case TI_73:		return 0x07;
		case TI_82: 	return 0x02;
		case TI_83: 	return 0x03;
		case TI_85:		return 0x05;
		case TI_86:		return 0x06;
		case TI_83P:
		case TI_83PSE:
		case TI_84P:
		case TI_84PSE:	return 0x23;
		default:		return ~0;
	}
}
...
static void link_RTS(CPU_t *cpu, TIFILE_t *tifile, int dest) {
	...
	strncpy(var_hdr.name, tifile->var->name, 8);
	...
	switch (err = setjmp(exc_pkt)) {
		case 0: {
			...
			for (page = 0; page < tifile->flash->pages; page++) {
				for (	offset = 0;
						offset < 0x4000 && offset < tifile->flash->pagesize[page];
						offset += 0x80 ) {
...
static LINK_ERR forceload_app(CPU_t *cpu, TIFILE_t *tifile) {
	u_char (*dest)[PAGE_SIZE] = (u_char (*)[]) cpu->mem_c->flash;
	if (dest == NULL)
		return LERR_MODEL;
	...
	for (	cpu->pio.link->vlink_send = 0;
			cpu->pio.link->vlink_send < 100;
			cpu->pio.link->vlink_send += 20 )
	{
